...
export function Layout() {
  ...
  return <div className={`shell${collapsed ? ' shell--rail' : ''}`}>
    <header className="mobile-header">
      <NavLink className="mobile-brand" to="/" aria-label="DairyIQ home">DAIRY<span>IQ</span></NavLink>
    ...
    <aside className={menuOpen ? 'open' : ''} aria-label="Application sidebar">
      <div className="sidebar-top"><NavLink className="brand" to="/" aria-label="DairyIQ home">DAIRY<span>IQ</span></NavLink><button className="rail-toggle" type="button" aria-expanded={!collapsed} onClick={() => setCollapsed((value) => !value)} title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}><span aria-hidden="true">{collapsed ? '»' : '«'}</span><span className="sr-only">{collapsed ? 'Expand sidebar' : 'Collapse sidebar'}</span></button></div>
      ...
      <NavLink className={({isActive}) => `settings-link${isActive ? ' active' : ''}`} to="/settings" title="Settings" aria-label="Settings"><span className="nav-icon" aria-hidden="true">⚙</span><span className="nav-label">Settings</span></NavLink>
